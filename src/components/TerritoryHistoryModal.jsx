...
export default function TerritoryHistoryModal({ territory, onClose, onUpdated, onPendingEdit }) {
  ...
  function openEdit() {
    setEditForm({ name: territory.name ?? '', number: territory.number ?? '', notes: territory.notes ?? '' })
    setFrontImageFile(null)
    setBackImageFile(null)
    setFrontPreview(territory.card_front_image_url ?? null)
    setBackPreview(territory.card_back_image_url ?? null)
    setError(null)
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-gray-900 rounded-t-3xl sm:rounded-3xl shadow-xl w-full sm:max-w-lg max-h-[92dvh] sm:max-h-[90vh] overflow-y-auto overscroll-contain flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        ...
        {showEditForm ? (
          ...
          <>
            {/* Edit header */}
            <div className="flex items-center justify-between px-4 sm:px-6 pt-4 sm:pt-6 pb-4 border-b border-gray-100 dark:border-gray-800 dark:border-gray-800 shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-base font-semibold text-gray-900 dark:text-gray-100">Editar território</span>
              </div>
              <button
                onClick={onClose}
                className="w-7 h-7 flex items-center justify-center rounded-full bg-[#f0f0f0] dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400 transition-colors cursor-pointer"
            ...
            {/* Edit form body */}
            <div className="flex-1 px-4 sm:px-6 py-4 sm:py-5 space-y-4 overflow-y-auto overscroll-contain">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Nome</label>
                  <input
                    value={editForm.name}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Número</label>
                  <input
            ...
            <div className="px-4 sm:px-6 pb-safe sm:pb-6 pt-4 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row gap-2 sm:justify-end shrink-0">
              ...
              <button
                onClick={cancelEdit}
                className="w-full sm:w-auto text-sm px-5 py-3 sm:py-2.5 rounded-xl bg-[#f0f0f0] dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 font-medium transition-colors cursor-pointer"
              >
                Cancelar
              </button>
            </div>
          ...
          <>
            ...
            <div className="flex items-center justify-between px-4 sm:px-6 pt-4 sm:pt-6 pb-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
              <div>
                <div className="flex items-center gap-2.5">
                  <span className="text-base font-semibold text-gray-900 dark:text-gray-100">{territoryTitle}</span>
                ...
                {!isAvailable && territory.assigned_to && (
                  <div className="mt-0.5">
                    <p className="text-sm text-gray-400 dark:text-gray-500">
                      Com <span className="font-medium text-gray-600 dark:text-gray-300">{territory.assigned_to}</span>
            ...
            {/* Body */}
            <div className="flex-1 px-4 sm:px-6 py-4 sm:py-5 space-y-4 sm:space-y-5 overflow-y-auto overscroll-contain">
              {/* Card images */}
              <div>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={label}>
                      <div className="relative">
                        <div
                          ...
                          className={`rounded-2xl overflow-hidden bg-[#f0f0f0] dark:bg-gray-800 ${src ? 'cursor-pointer' : ''}`}
                        >
                          {src ? (
                            <img src={src} alt={`${territoryTitle} — ${label}`} className="w-full object-cover aspect-[5/3]" />
                          ) : (
                            <div className="aspect-[5/3] flex items-center justify-center text-gray-300 dark:text-gray-600 text-xs">Sem imagem</div>
                          )}
                        </div>
                        {src && (
                          <button
                      ...
                      <p className="text-center text-xs text-gray-400 dark:text-gray-500 mt-1.5 font-medium">{label}</p>
                    </div>
                  ))}
                </div>
                {(frontPreview || backPreview) && (
                  <div className="mt-2.5 flex gap-2">
                    <button
                      ...
                      className="flex-1 text-sm py-2 rounded-xl bg-[#f0f0f0] dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors font-medium flex items-center justify-center gap-1.5 cursor-pointer"
                    >
              ...
              {/* Notes */}
              {territory.notes && (
                <div className="bg-[#f0f0f0] dark:bg-gray-800 rounded-2xl px-4 py-3">
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Observações</p>
                  <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{territory.notes}</p>
                </div>
              )}
              ...
              <div>
                ...
                {loadingHistory ? (
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto overscroll-contain">
                    {history.map(entry => (
                      <div key={entry.id} className="flex items-start justify-between text-sm bg-[#f0f0f0] dark:bg-gray-800 rounded-xl px-3 py-2.5">
                        <div>
                          <p className="font-medium text-gray-800 dark:text-gray-200">{entry.person_name}</p>
              ...
              {/* Assign form */}
              {showAssignForm && (
                <form onSubmit={handleAssign} className="bg-[#f0f0f0] dark:bg-gray-800 rounded-2xl p-4 space-y-3">
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Atribuir território</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Nome da pessoa</label>
                      <input
                    ...
                    <div>
                      <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Data de designação</label>
                      <input
                        type="date"
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-end gap-2 pt-1">
                    ...
                    <button
                      ...
                      className="w-full sm:w-auto text-sm px-4 py-2.5 sm:py-2 rounded-xl bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 font-medium transition-colors"
                    >
                      Cancelar
                    </button>
                  </div>
              ...
              {/* Deliver form */}
              {showReturnForm && (
                <form onSubmit={handleDeliver} className="bg-red-50 dark:bg-red-900/20 rounded-2xl p-4 space-y-3">
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                    Entregar — {territory.assigned_to}
                  </h3>
                  <div>
                    <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Data de entrega</label>
                    <input
                      type="date"
                      value={returnDate}
            ...
            {/* Footer */}
            <div className="px-4 sm:px-6 pb-safe sm:pb-6 pt-4 border-t border-gray-100 dark:border-gray-800 flex items-center gap-2 shrink-0">
              {/* Edit button — bottom left */}
              <button
                onClick={openEdit}
                className="text-sm px-4 py-2.5 rounded-xl bg-[#f0f0f0] dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 font-medium transition-colors cursor-pointer"
              >
                Editar
              </button>
